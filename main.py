...
df["대표국가"] = df["nation"].astype(str).str.split("|").str[0].str.strip()

counted = (
    df.groupby(["대표국가", "genre"], as_index=False)
    .agg(편수=("movieNm", "count"))
)

fig7 = px.sunburst(counted, path=["대표국가", "genre"], values="편수")
...
# ==============================================================
# 그래프 8. 10위권 유지 기간과 총 관객의 관계 (산점도)
# ==============================================================
st.header("그래프 8. 10위권 유지 기간과 총 관객의 관계")
...
fig8.update_traces(
    hovertemplate="<b>%{hovertext}</b><br>10위권 유지일수: %{x}일<br>총 관객: %{y:,}명<extra></extra>",
)
fig8.update_layout(
    margin=dict(t=20, b=20, l=20, r=20),
    xaxis_title="10위권 유지 일수",
    yaxis_title="총 관객 수",
    legend_title_text="장르",
)
...
# 상관관계 계산 (10위권 유지일수가 짧을수록 총 관객이 적은 경향이 있는지 확인)
corr8 = df["days_in_top10"].corr(df["total_audi"])

st.markdown(
    f"10위권 유지 일수와 총 관객 수의 상관계수는 **{corr8:.2f}**입니다. "
    f"{'값이 클수록(1에 가까울수록) 오래 10위권에 머문 영화가 관객도 많다는 뜻이고, ' if corr8 > 0 else ''}"
    "그래프에서 점들이 왼쪽 아래(유지일수 짧고 관객 적음)에 몰려 있는지, "
    "아니면 유지일수가 짧아도 관객이 많은 예외가 있는지 살펴보세요."
)
